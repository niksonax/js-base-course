...
let titleRatingArr = watchList.map(function (item) {
  return item.Title + ". Rating: " + item.imdbRating;
});
console.log(titleRatingArr);
...
let nolanFilms = watchList.filter(function (item) {
  return item.Director === "Christopher Nolan";
});
console.log(nolanFilms);
let nolanFilmsRating = nolanFilms.map(function (item) {
  return item.imdbRating;
});
console.log(nolanFilmsRating);
let AverageRating = nolanFilmsRating.reduce(function (sum, currentItem) {
  return Number(sum) + Number(currentItem);
});
AverageRating /= nolanFilmsRating.length;
